...
bool integerToScoreCoupons(struct __ribbonConstructor *__rc, __ribbonType n, __ribbonScoreCouponType *scoreCoupons, bool nullByte){
    if((__rc == NULL) || (scoreCoupons == NULL)) return false;
    ...
    for(__ribbonType i = 0; i < __rc->_couponsValues; i++){
        ...
        if(error){
            for(__ribbonType n = 0; n < __rc->_couponsValues; n++){
                scoreCoupons[n] = __rc->_scoreCoupons[0];
...
int main(int argc, char *argv[]){
    ...
    __ribbonScoreCouponType itsc[7 + 1];
    __ribbonType scti = scoreCouponsToInteger(&__rc, "FFFEEDA", &sctiError);

    if(!integerToScoreCoupons(&__rc, 727, itsc, true)){
